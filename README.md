...
показывает, куда попала каждая ступень. Внизу рядом — **Загрузка** (proxy,
сложен до нажатия на заголовок) и **Пути**: путь к Python и каталог моделей.
На узком экране области идут одна под другой.
