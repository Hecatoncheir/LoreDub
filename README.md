...
сеанс, с последней их репликой. У каждого голоса есть выбор **Озвучивать
как**: любому из них можно назначить записанного персонажа, и со следующей же
реплики этот голос будет звучать его голосом и, с **Голосом оригинала**, его
тембром. Заменить можно и голос, который LoreDub завёл сам, и узнанную
карточку — так безымянный стражник получает голос вашего Стражника. Выбор
хранится для этой игры и действует после перезапуска; **Как услышано**
возвращает голос себе, а замену, назначенную на самой карточке персонажа,
строка подписывает как «По карточке».
...
персонажам поговорить, раздайте им голоса — и лишь потом запускайте перевод:
голоса, заведённые при прослушивании, попадают в банк игры под теми же
номерами, так что назначения остаются в силе. Перевод, запущенный поверх
...
<p align="center">
  <img src="docs/screenshots/snapshot-ru.png" width="900" alt="Экран «Фрагмент»: выделенные куски текста с переводом">
</p>

Разовый перевод текста с экрана — записки, подсказки, строки меню, всего, что
игра не произносит. Нажмите **Запустить**: загружаются только переводчик и
голос, без распознавания речи и без выбора процесса. Затем в игре удерживайте
клавишу выделения (по умолчанию Ctrl+Alt+S) — экран слегка затемнится, —
обведите текст мышью и отпустите клавишу. Рамка исчезает, текст из неё
читается распознаванием Windows, переводится и озвучивается, а на вкладке
появляется оригинал с переводом. Esc или правая кнопка мыши отменяют
выделение.

Клавиша работает и во время **Эфира**, на уже загруженных моделях; запуск
**Эфира** при работающем выделении сначала останавливает выделение. Сочетание
меняется в **Настройках** → **Горячие клавиши**. **Язык текста** на вкладке —
английский (он переводится) или язык озвучки (он озвучивается без перевода).
Рамка рисуется поверх окон, поэтому игра должна идти в оконном или
...
<p align="center">
  <img src="docs/screenshots/characters-ru.png" width="900" alt="Экран «Персонажи»: карточки записанных голосов и пакет под ними">
</p>
...
Под карточками — **пакеты**. **Создать пакет** заводит область, её имя правится
...
«Whisper» — это дубляж со звука; протяните **Текст с экрана** прямо в
«Перевод» — это режим субтитров, и «Whisper» помечается как обойдённый, но
остаётся на месте, чтобы маршрут можно было вернуть одной связью. Маршрута,
которого у движка нет, нарисовать нельзя: связь не создаётся, а под панелью
появляется строка с причиной. Две готовые раскладки — **Дубляж со звука** и
**Субтитры с экрана** — ставят маршрут и раскладывают ноды заново.
...
гнезда: **Персонаж** на входе, которым она подключена к общему голосу,
необязательный **Голос** на входе и **Голос** на выходе. Связь от одной
карточки к другой значит, что реплики первой произносит вторая: протяните
«Голос» Стражника в «Голос» Кузнеца — и роль Стражника читает Кузнец, своим
голосом и своим тембром, хоть мужским за женскую роль, хоть наоборот. Из
голоса карточки выходит ровно один путь: отданный другой карточке, он идёт
туда, а не в сведение, — до сведения его доносит тот, кто взял роль.

Это та же замена, что стоит в карточке на экране **Персонажи**: она действует
во всех играх, а выбор в **Голосах сцены** перебивает её для текущей игры.
Двух персонажей, которые читали бы друг друга, соединить нельзя. Имя правится
прямо в панели ноды, а крестик на самой карточке убирает её с холста, не
трогая ни карточку, ни назначенную ей замену.
...
Громкость оригинала в звуковом режиме не опускается ниже 10%. Windows отдаёт
захват после громкости сессии, поэтому, приглушая игру, LoreDub приглушает и
то, что слышит сам: на нуле он не услышал бы ни одной реплики. В режиме
субтитров ноль доступен — звук там не захватывается, и игру можно заглушить
совсем.
...
Язык текста выбирается на **Эфире** вместо **Определять язык**: английский
переводится, а текст на языке озвучки — например, русские субтитры при русской
озвучке — озвучивается как есть, без перевода.
