...
- **Шесть языков озвучки:** русский, немецкий, испанский, французский,
  украинский и английский. Английскому переводчик не нужен вовсе — whisper
  и так отдаёт английский, — поэтому он идёт одним голосом и заводится
  быстрее прочих.
...
Сюда записывается то, чего не знает никакая модель. Три списка, потому что
запись каждого вида достаёт до реплики в свой момент и не может сделать работу
другого.

**Фразы** — целая реплика с вашим переводом. Она отвечается до того, как
спросят модель: «Fire in the hole!» больше не «Огонь в дыру!». **Имена** —
слово, которое переводчик оставил латиницей; вместо автоматической
транслитерации подставляется ваше написание, и только внутри реплики, не
поверх неё: модель имя, которое перевела, обычно ещё и склоняет правильно.
**Слова** — единственная запись, которая смотрит на язык озвучки, а не на
английский: если модель назвала место «Хранилищем», а вы зовёте его
«Убежищем», это правится здесь. Меняется слово целиком.

Запись обычно делается не тут, а на **Эфире**: реплика слышна один раз, и
кнопка рядом с её задержкой открывает перевод для правки, а исправленное
кладёт сюда как фразу. Словарь один на все игры, как и карточки персонажей, и
действует со следующей реплики, а не со следующего запуска.

Сбоку от списков — **пакеты**; в узком окне они уходят под списки. Пакет
это способ прочитать словарь, а не место,
куда записи переносятся: запись остаётся в своём списке, даже когда пакет её
называет, и одна запись может лежать в нескольких пакетах. Записи
перетаскиваются в пакет за рукоятку слева от строки, а обратно — за сам
значок в пакете или крестиком на нём.

Переключатель **Активен** решает, что читает озвучка. Пока не включён ни один
пакет, она сверяется со всем, что записано. Как только включён хотя бы один —
только с включёнными, и это главное, ради чего пакеты нужны: словарь одной
игры не тащит за собой имена другой. Включённых может быть несколько, их
записи складываются.

LoreDub приносит с собой два пакета. **«LoreDub · English → Русский»** — 74 фразы,
на которых переводчик ошибается: «Weapons free.» он переводил как «Оружие
бесплатно.», «Tango down.» — «Танго вниз.», «Objective updated.» — «Объектив
обновлен.». То, что модель и так говорит верно, в пакет не вошло, потому что
запись, которая никогда не срабатывает, — мусор в списке. Пакет приходит
выключенным и предлагается один раз: если его удалить, он не вернётся.

Второй — **«LoreDub · Крылатые фразы»**, 27 реплик, которые игрок знает
наизусть. Тут мало быть правым буквально: строку помнят по локализации, и
свежий перевод звучит мимо, каким бы верным он ни был. «Requiescat in
pace.» модель переводила как «Требуем в темпе.», «The Vault Dweller has
returned.» — «Домой Домовладельцы вернулись.», а про колено — «взял стрелу
в колено» вместо «пока мне не прострелили колено».

В нём только фразы, и это тоже замер, а не выбор. Запись-имя достаёт лишь до
латиницы, которую переводчик оставил, — на шестнадцати именах в двух разных
предложениях не сработала **ни одна**: модель передаёт все имена сама.
Запись-слово сличается с языком озвучки и заменила бы слово везде, где оно
встретится, так что и то и другое место имеет в пакете под конкретную игру, а
не в общем.

Пакет выгружается в файл вместе с записями, которые в нём лежат, — чужой файл
достаточно загрузить, и записи появятся и в пакете, и в списках. Пришедший
пакет всегда выключен: файл не должен молча менять то, с чем сверяется
следующая реплика. Удаление пакета оставляет записи на месте, а удаление
записи убирает её из всех пакетов сразу.
...
поток**, **Whisper**, **Перевод**, **Реплики**, **Сведение** и **Поток**,
...
Связь с **Переводом** снимается тоже, и означает она озвучку на английском.
Whisper и так отдаёт английский, поэтому текст идёт от него прямо в «Реплики»,
а нода перевода гаснет с подписью «Переводить нечего» — как и язык
«Английский», выбранный на «Моделях»: это одно и то же состояние, а не две
настройки. Линия, проведённая обратно, ставит перевод на первый скачанный
язык, у которого переводчик есть; если такого нет, связь отклоняется с
просьбой сначала скачать язык. Как и маршрут, это нельзя переключить на ходу:
модели загружены под ту сессию, что идёт.

Клик по ноде открывает её настройки в панели у правого края холста, во всю
его высоту: игра и источник звука, модель распознавания, язык перевода, голос
и скорость озвучки, громкость оригинала, наложение реплик и устройство, на
котором считается этот шаг. Панель носит лицо выбранной ноды — её иконку в
шапке и её цвета, — поэтому видно, чьи это настройки: у концов конвейера она
оранжевая, у карточки персонажа тёмная. Это те же настройки, что на других
экранах, — схема ничего не хранит отдельно, поэтому разойтись им не с чем.

<p align="center">
  <img src="docs/screenshots/inspector-ru.png" width="900" alt="Панель ноды на «Схеме»: выбрана карточка персонажа, панель стоит у правого края в её цветах">
</p>
...
персонаж никогда не перебивает сам себя. Снятая связь карточки со
**Сведением** убирает из сведения её одну: карточка остаётся на холсте, но
гаснет, свой голос никому не отдаёт и сама читается как слышно — кто её
заменяет, ждёт в ней и возвращается вместе со связью. Остальных карточек это
не трогает, а персонаж, которого на холсте нет, озвучивается как и раньше. Это
действует сразу, даже если перевод уже идёт или стоит на паузе: работающему
воркеру говорят, а не перезапускают его.
...
Пунктир от **Персонажей** ноды «Реплики» ко входу **Персонаж** — это пометка,
а не маршрут: она говорит, что этого персонажа можно встретить в оригинальной
...
карточке перезаписывает схему тем, что сейчас на холсте, переименовывает её,
удаляет и выгружает в файл, а **Загрузить схему из файла** читает её обратно:
схема с тем же идентификатором ложится на ту, из которой сделана, а не рядом. Маршрут, как и везде, закреплён на время
...
У английского в плитке одна половина: переводчика у него нет, и качать нужно
только голос.
...
| Английский | 48 голосов `en_*` | 57 голосов `en_*` | да |
...
Переключатель **«Накладывать реплики разных персонажей»** (выключен по
умолчанию) позволяет новой реплике другого персонажа начаться сразу, пока
предыдущая ещё звучит, — так озвучка меньше отстаёт в быстрых диалогах. По
умолчанию он выключен: два голоса одновременно — это две реплики, которые
игроку приходится слушать разом, а понять сцену проще, когда она читается
подряд.
...
Переводчик работает не через transformers, а через **CTranslate2**: те же
веса, квантованные в int8. Замер на двенадцати игровых репликах, 12 потоков,
обе стороны прогреты:

| | transformers | CTranslate2 int8 |
| --- | --- | --- |
| медиана на реплику | 262 мс | 107 мс |
| загрузка модели | 4,1 с | 0,3 с |
| на диске | 461 МБ | 234 МБ |

Текст совпал дословно в одиннадцати случаях из двенадцати. В двенадцатом int8
перевёл ближе к оригиналу: «Три лета назад колодец высох» против «Три года
назад» для *The well ran dry three summers ago*.

Конвертация сделана один раз здесь, а не на машине игрока: ей нужен более
свежий transformers, чем несёт runtime, и она стоила бы каждому минуту и пик
памяти на загрузку в float32. Поэтому переводчики скачиваются из релиза этого
проекта, а не от авторов — `scripts/convert_translators.py` их собирает и
печатает готовые записи каталога. Лицензия CC-BY-4.0 это разрешает и просит
взамен две вещи: указать авторов и сказать, что изменено, — то и другое лежит
файлом NOTICE рядом с весами. Каждый файл вдобавок закреплён размером и
SHA-256, чего с первоисточником быть не могло: своих контрольных сумм
OPUS-MT не публикует.

Английской озвучке переводчик не нужен вовсе: whisper запускается с `-tr` и
отдаёт английский, что бы игра ни говорила, — реплика доходит до голоса уже
на нём. Поэтому язык «Английский» в каталоге состоит из одного голоса, сессия
поднимается без Marian, и слабой машине это единственный язык, который ей по
силам. В пакете Silero `v3_en` 118 голосов; в списке 105 — 57 женских и 48
мужских, — а тринадцать, чья высота легла между порогами, не названы вовсе,
как и везде в каталоге.
...
| Озвучка (Silero) | да, тот же пакет torch | нет: у torch нет Vulkan-бэкенда | всегда |
...
Озвучка вынесена в отдельную строку таблицы затем, чтобы её можно было
оставить на процессоре: видеокарта ускоряет её мало. Замерено на девяти
репликах, RTX 3080 Ti, двенадцать потоков: на процессоре время растёт с длиной
строки — 23 мс на одно слово, 53 на одиннадцать, 66 на четырнадцать, — а на
CUDA держится 20–26 мс при любой длине, потому что это почти целиком запуск
ядер. Выигрыш выходит около 30 мс на реплике, и платится за него секундой на
старте сессии (первая фраза после загрузки — 2.1 с против 1.1) и 350 МБ
видеопамяти. Пересылка результата обратно тут ни при чём: она занимает 0.1 мс.
Обе стадии произносят одну и ту же реплику с точностью до отсчёта, расходясь
только так, как расходятся числа с плавающей точкой. Карта, которой не хватило
памяти, тихо возвращает голос на процессор — без него дублировать нечем.
...
небольшой и входит в установщик, докачивать его не требуется. В копии,
собранной без него, ячейка Vulkan у Whisper погашена, а в подсказке написано,
почему: скачать эту сборку неоткуда, она приходит вместе с установщиком.
...
- **Второй движок синтеза — Piper.** У Silero шесть языков, у Piper — 57, и
  среди них итальянский, польский, португальский, турецкий, китайский,
  нидерландский, чешский, шведский и арабский, то есть те, в которые Marian
  переводит уже сейчас. Голос там — файл `.onnx` на 60 МБ, а не пакет на язык,
  так что игрок качает ровно тот голос, который слушает; темп задаётся синтезу
  (`length_scale`), а не растягиванием готовой волны; частота 22.05 кГц
  потребует пересчёта. Конвертер тембра переезд переживёт — он работает по
  волне, а не по модели синтеза. Ограничение одно и важное: `piper1-gpl` под
  GPL-3, потому что внутри espeak-ng, поэтому звать его можно только отдельным
  процессом (`python -m piper.http_server`), как уже зовётся whisper, — импорт
  в воркер сделал бы GPL всю сборку. Русский остаётся на Silero: из четырёх
  русских голосов Piper два под CC0 и оба мужские, женский один и без
  указанной лицензии.
- **Выпуск со сборкой Vulkan, то есть для AMD и Intel.** Сейчас установщик
  уезжает без неё: `scripts/build_setup.ps1` зовёт
  `prepare_windows_runtime.ps1` без `-RequireVulkan`, а на раннере нет Vulkan
  SDK, поэтому шаг пропускается с одним предупреждением в логе. У владельцев
  AMD и Intel из-за этого распознавание остаётся на процессоре, а ячейка
  Vulkan в таблице устройств погашена и говорит, что этой сборки в копии нет.
  Чинится двумя правками. Первая — шаг в `.github/workflows/release.yml`,
  ...
  Переменную приходится выставлять руками: установщик пишет её в систему, а
  шаг workflow получает окружение, снятое при старте задания. Вторая правка —
  флаг у вызова в `scripts/build_setup.ps1`:
  ...
  Флаг нужен сам по себе: без него пропуск остаётся предупреждением, и релиз
  снова уедет без Vulkan, никого не предупредив.

  Чего это стоит. CI по веткам (`windows.yml`) не затрагивается — установщик
  он не собирает. В релизном задании добавятся установка SDK и сборка
  `whisper.cpp` с `-DGGML_VULKAN=ON`, где компилируются ещё и шейдеры; оценка
  10–20 минут на раннере, не замерено. Платится один раз: `Build-VulkanWhisper`
  пропускает сборку, если бинарник уже лежит в кеше `LoreDubBuildCache`. Кеш
  при этом стоит сузить — иначе в него попадёт всё дерево `build-vulkan` на
  сотни мегабайт при лимите 10 ГБ на репозиторий, а нужны там только готовые
  `.exe` и `.dll`. Кроме SDK сборке нужны CMake и MSVC; на `windows-latest`
  они есть, но проверить стоит.

  Что это даёт и чего не даёт. На видеокарту переедет только распознавание —
  зато самая дорогая стадия: `medium-q5` на сегменте 4.7 с считался 5.0–6.0 с
  на CUDA против 10.9–11.3 с на процессоре. Перевод, озвучка и OpenVoice
  останутся на процессоре: они живут в torch, у которого под Windows нет
  Vulkan-бэкенда, а ROCm под Windows не собирают. Игроку после установки
  качать ничего не придётся — бинарник едет внутри установщика, единицы
  мегабайт, и приложению довольно того, что файл лежит на месте. От машины
  нужен только загрузчик `vulkan-1.dll`, который ставит драйвер видеокарты.
...
Собственный код проекта распространяется по лицензии MIT. Ни модели, ни runtime
в репозитории и в установщике не лежат — приложение скачивает их на машину
игрока, и каждый из них сохраняет лицензию своего правообладателя.

| Что скачивается | Лицензия | Коммерческое использование |
| --- | --- | --- |
| Код LoreDub | MIT | да |
| whisper.cpp, сборки CPU, Vulkan и CUDA | MIT | да |
| Веса Whisper (`ggml-*.bin`) | MIT | да |
| Marian `opus-mt-tc-big-en-zle` (русский), `opus-mt-en-de` | CC-BY-4.0 | да, с указанием авторства |
| Marian `opus-mt-en-es`, `opus-mt-en-fr`, `opus-mt-en-uk` | Apache-2.0 | да |
| Голоса Silero (`v5_3_ru`, `v3_de`, `v3_es`, `v3_fr`, `v4_ua`) | **CC BY-NC-SA 4.0** | **нет** |
| OpenVoice V2, конвертер тембра | MIT | да |
| torch, в том числе сборка с CUDA | BSD-3-Clause, библиотеки NVIDIA — по EULA NVIDIA | да |
| transformers, sentencepiece | Apache-2.0 | да |
| sacremoses | MIT | да |
| Встроенный Python 3.11 | PSF License | да |
| Шрифты Nunito, Nunito Sans, JetBrains Mono | OFL 1.1 | да |

Исключение в этом списке одно — голоса Silero. **NC** означает «только
некоммерческое использование», **SA** — что производные от весов (дообучение,
конвертация в другой формат, квантизация) обязаны выходить под той же лицензией.
Для самого LoreDub это ничего не меняет: приложение бесплатное и весов не
распространяет. Но если озвучка приносит деньги — монетизированный стрим,
студийная работа, платный продукт на этом коде, — на голос Silero нужна
отдельная лицензия от правообладателя; остальной стек её не требует.
